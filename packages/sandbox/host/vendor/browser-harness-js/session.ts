...
import { bindDomains, type Domains, type Transport } from './generated';
...
/**
 * Scope CDP calls to one `browser_execute` invocation. When the call times out
 * the execution object is flipped inactive; subsequent CDP calls from the
 * orphaned snippet reject instead of mutating a browser the next call now owns.
 */
export const withSessionExecution = <T>(execution: SessionExecution, run: () => T): T =>
  sessionExecution.run(execution, run);
...
export class Session implements Transport {
  ...
  /**
   * Subscribe to successful call results. Used to tap `Page.captureScreenshot`
   * responses so `browser_execute` can attach images without the snippet
   * decoding them itself. Returns an unsubscribe fn.
   */
    ...
    if (typeof m.id === 'number') {
      ...
      if (m.error) p.reject(new CdpError(m.error.code, m.error.message, m.error.data));
        ...
        for (const fn of this.callResultListeners) {
          try { fn(p.method ?? '', p.params ?? {}, m.result); } catch { /* ignore */ }
        }
      }
    } else if (m.method) {
